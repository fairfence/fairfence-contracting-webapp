...
// Note: Database uses UUID but this table might still use varchar in actual DB - check migrations
...
export const siteContent = pgTable("site_content", {
  ...
  updatedBy: uuid("updated_by"), // References auth.users, not local users table
...
export const images = pgTable("images", {
  ...
  uploadedBy: uuid("uploaded_by"), // References auth.users, not local users table
...
export const fenceLines = pgTable("fence_lines", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  surveyId: uuid("survey_id").notNull().references(() => siteSurveys.id, { onDelete: "cascade" }),
